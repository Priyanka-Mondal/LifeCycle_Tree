...
public class BlameAssign {
  ...
  public void printMap(HashMap<Integer, Pair> tree2)
  {
  ...
  public void printLock(State2 state, Value v  ,int dest)
  {
	  //System.out.println("source node:"+v.node+" "+"source line:"+v.line+"  dest:"+dest);
  }
  ...
  public void FindPath(int count, Edge e ,int number, String args0 , FileWriter fw) throws IOException
  {
  ...
  public void reverseStack(Stack<Edge> s, Stack<Edge> Backup)
  {
	  Stack<Edge> Dummy = new Stack<Edge>();
	  while(!s.isEmpty())
	  {
  ...
  public  void phaseTree(String args) throws IOException
  {
	  BufferedReader br = new BufferedReader(new FileReader(args));
	  //String filename = "output.txt";
	  BufferedWriter brw = new BufferedWriter(new FileWriter("output.txt"));
	  
	 // String msg = null; //first method
	  String line;
	  int flag = 0;	
	  countLine =0;
	  try 
			  ...
			  else if(line.contains(" CALL "))
			  {
				  ...
				  if(Tree.containsKey(p1))
				  {
					  ...
					  else if(pair.source==t1)
					  {
						  ...
						  {
	        					//System.out.println(line);
		        				//String temp = t1.substring(4);
		    	        		//int tt =  Integer.valueOf(temp);
							  if(pair.parent!=0)
							  {
								  pair2=Tree.get(pair.parent);
  ...
  public void clearState()
{
  ...
  public  void phaseCheck(int number , String args0) throws IOException 
  {
	  ...
	  String filename = args0.concat("output");
	  ...
	  try 
	  {
		  ...
		  while(line!=null)
		  {
			  ...
			  //System.out.println("fk "+fk.tid +" "+fk.treeRootid);
		      ...
		      /*else if (line.contains("endAtomic")) //RULE EXIT
			  {
				  state.C.put(tid,-1);
				  flagAtomic.put(fk,0);
			  }*/
			  ...
			  if(line.contains("ATTACH-Q"))
			  {
				 ...
				 /* System.out.println("Printing Queue");
				  Set<Integer> keys = AttachQ.keySet();
				  for(Integer kk:keys)
				  System.out.println(kk+" "+AttachQ.get(kk));*/
			  }
			  if (line.contains(" CALL ")) 
			  {
				  LinkedHashSet<Integer> ls = new LinkedHashSet<Integer>();//to store anscestors of a node
				...
				else if (line.contains("THREADINIT"))  // *** THERE IS NO FORK FOR THE UI THREAD.. WHAT TO DO???
				{
					...
					//not needed
					/*if(state.L.containsKey(tid) && state.L.get(tid) != -1 && state.L.get(tid) != nodeId)
					{
						if (state.H.containsKey(nodeId)) 
						{
							ls.addAll(state.H.get(nodeId));
						}
						ls.addAll(state.H.get(state.L.get(tid)));
						ls.add(state.L.get(tid));
						state.H.put(nodeId,ls);
						//print(state,line,state.L.get(tid),nodeId);
					}*/
					 v = new Value();
					 v.node = nodeId;
					 v.line = lineNum;
				...
				else if (line.contains("JOIN")) //check syntax of join 
				{
					...
					{// tid changed to ctid
						...
						{														
							...
							EdgeStore.put(++EdgeCount,e);
							CausalEdges.add(e);
							System.out.println("-----------------CYCLE for THREADEXIT-JOIN EDGE!! "/*+state.U.get(lockObj).node+" "+nodeId*/);
							...
							FindPath(EdgeCount, e, number,args0,fw);
						}
						////////////////////////
						else
						{
				...
				else if (line.contains("LOCK") && !line.contains("UNLOCK") || ((line.contains("meth:lock") || line.contains("meth:tryLock"))&& line.contains("METHOD EXIT") && !line.contains("$"))) 
				{
					...
					{									
						...
						{														
							...
							EdgeStore.put(++EdgeCount,e);
							System.out.println("-----------------CYCLE for RELEASE_ACQUIRE EDGE!! "/*+state.U.get(lockObj).node+" "+nodeId*/);
						...
						state.H.put(nodeId,ls);
							printLock(state,state.U.get(lockObj),nodeId);
				...
				else if (line.contains("READ"))  
				{
					...
					if (state.W.containsKey(k) && state.H.containsKey(state.W.get(k).node) && state.H.get(state.W.get(k).node) != null && nodeId != state.W.get(k).node)
					{
						...
						if(state.H.get(state.W.get(k).node).contains(nodeId))
						{
							...
							EdgeStore.put(++EdgeCount,e);
							System.out.println("----------------------CYCLE for WRITE-READ EDGE!! "/*+objId+" field:"+fieldId+" (src:"+state.W.get(k).node+","+state.W.get(k).line+") (dest:"+nodeId+","+lineNum+")"*/);
						...
						else
						{
							...
							/*if(EdgeCount >0)
							{
								System.out.println("EIGHTTH"+EdgeStore.get(EdgeCount)+" "+EdgeCount);
							}*/
							ls.addAll(state.H.get(state.W.get(k).node));
							ls.add(state.W.get(k).node);
						}
						state.H.put(nodeId,ls);
						//if(objId.equals("0x410e22a0") && fieldId.equals("104")&& nodeId==1034)
						//printLock(state,state.W.get(k),nodeId);
						//print(state,line,state.W.get(k).node,nodeId);
						entered =1;
					}
				...
				else if (line.contains("WRITE")) //check: what about read-static
				{
					...
					if (state.W.containsKey(k) && state.H.containsKey(state.W.get(k).node) && state.H.get(state.W.get(k).node) != null && nodeId != state.W.get(k).node)
					{
						...
						if(state.H.get(state.W.get(k).node).contains(nodeId))
						{
							...
							ls.add(state.W.get(k).node);
							ls.remove(nodeId);
							System.out.println("--------------------CYCLE for WRITE WRITE EDGE!! "/*+objId+" "+fieldId+" src:"+state.W.get(k).node+" dest:"+nodeId*/);
							System.out.println("BLAME EDGE:"+"[("+state.W.get(k).node+","+state.W.get(k).line+"),("+nodeId+","+lineNum+")]");
							System.out.println("BLAME TRANSACTION: "+nodeId);
							fw.write("\nBLAME EDGE:"+"[("+state.W.get(k).node+","+state.W.get(k).line+"),("+nodeId+","+lineNum+")]");
							fw.write("\nBLAME TRANSACTION: "+nodeId);
					...
					/*if(objId.equals("0x4110bec0") && fieldId.equals("80"))
					{
						...
						//print(state,line,state.W.get(k),nodeId);
							...
							System.out.println(line+" "+nodeId);
				        ...
				        if ((r.getObjectId().equals(objId) )&& (r.getFieldId().equals(fieldId)) )
				        {
				        	if (state.H.containsKey(state.R.get(r).node) && state.H.get(state.R.get(r).node) != null && nodeId != state.R.get(r).node)
				        	{
				        		...
				        		if(state.H.get(state.R.get(r).node).contains(nodeId))
								{
									...
									ls2.remove(nodeId);
									System.out.println("----------------------CYCLE for READ-WRITE EDGE!!  "/*+objId+" "+fieldId+" src:"+state.R.get(r).node+" dest:"+nodeId*/);
									...
									FindPath(EdgeCount, e, number,args0,fw);
									//print(state,line,state.W.get(k),nodeId);
									//print(state,line,state.R.get(r),nodeId);
								}
				        		else
				        		{
				        			v = new Value();
				        		...
				        		//printLock(state,state.R.get(r),nodeId);
  ...
  public void markCalls(String args0) throws IOException
  {
	  BufferedReader br = new BufferedReader(new FileReader(args0));
	  BufferedReader br1 = new BufferedReader(new FileReader(args0));
	  String out = args0+"_output";
	  ...
	  try
	  {
		  ...
		  while(line != null)
		  {
			  ...
			  else if(line.contains("TRIGGER-LIFECYCLE"))
			  {
				  ...
				  {
					 // System.out.println("id:"+id+" ct:"+currentTask.get(tid));
					  LifeValues lv = new LifeValues();
					  lv = TaskM.get(currentTask.get(tid));
		 ...
		 // printCall(TaskM);
		  ...
		  while(line != null)
		  {
			 ...
			 flag = 0;
			  if(line.contains(" CALL ") && TaskM.containsKey(taskid) && TaskM.get(taskid).component != null)
			  {
				  flag = 1;
				  currentTask.put(tid, taskid);
				  line = excludeLineNum(line);
	  ...
	  {
		  //System.out.println("im in finally of markcall");
		  br.close();
		  br1.close();
		  brw.close();
	  }
  ...
  public void printCall(HashMap<Integer,LifeValues> TaskM)
  {
	  System.out.println("inside printCall");
	  Set<Integer> keys = TaskM.keySet();
	  for(Integer kk:keys)
	  {
		  if(TaskM.get(kk).component != null)
		  System.out.println("msg:"+kk+" "+TaskM.get(kk).toString());
	  }
...
/*public void clearStateLife()
{
  ...
  public void checkLifecycles(String args0) throws IOException
  {
	  //clearState();
	  int intentId;
	  int id = 0;
	  ...
	  try
	  {
		  while(line!= null)
		  {
			  ...
			  if(line.contains(" CALL "))
			  {
				  //System.out.println("in call "+taskid);
				  taskMap.put(tid, taskid);
			  }
			  int getId;
			  if(TaskM.containsKey(taskMap.get(tid)))
			  {
				  getId = TaskM.get(taskMap.get(tid)).id;
				  //System.out.println(line+"  "+getId+"  "+"getId"+" "+tid);
				  
			  }
			  else
				getId = -1;
		flagKeyLife fk = new flagKeyLife(tid,getId);
			 
		if(!(firsttimeLife.containsKey(fk)) && line.contains("beginAtomic"))
		  {
			  firsttimeLife.put(fk, 0);
		  }	
		      if (line.contains("beginAtomic") && firsttimeLife.get(fk)==0) //RULE ENTER :first task with begin atomic annotation
		      {
		    	  LinkedHashSet<Integer> ls = new LinkedHashSet<Integer>();//to store anscestors of a node
		    	  Value v = new Value();
		    	  v.node = ++ countNode;
		    	  ...
		    	  //lv.node = countNode;
		    	  //TaskM.put(taskid , lv);
		    	  RecNode.put(TaskM.get(taskid).id,countNode);
		    	  //System.out.println("firsttime "+line+" "+countNode);
		    	  flagKeyLife fk1 = new flagKeyLife(tid,TaskM.get(Tree.get(taskid).tree).id);	
				  //System.out.println(line+" "+v.node);
				  //System.out.println(taskid+" taskid "+fk1.toString()+" fk1 and TaskM "+TaskM.toString());
				 
		      }
		      else if (line.contains("beginAtomic")  && firsttimeLife.get(fk) == 1) //RULE ENTER: child beginAtomic																task of Atomic block
		      {  
		    	  //LinkedHashSet<Integer> ls = new LinkedHashSet<Integer>(); //to store anscestors of a node
		    	  Value v = new Value();
		    	  v.node = RecNode.get(TaskM.get(taskid).id);
		    	  v.line = lineNum;
		    	  state.C.put(tid,v);
		    	  flagAtomicLife.put(fk, 1);
		    	  state.L.put(tid,v);  
		    	 // System.out.println("NOT firsttime "+line+" "+countNode);
		    	  flagKeyLife fk1 = new flagKeyLife(tid,TaskM.get(Tree.get(taskid).tree).id);	
				  //System.out.println(line+" "+v.node);
				  //System.out.println(taskid+" taskidfk=1 "+fk1.toString()+" fk1 and TaskM "+TaskM.toString());
				 
		      }
			  if ((!flagAtomicLife.containsKey(fk) || flagAtomicLife.get(fk) != 1) && (!outside.containsKey(taskMap.get(tid)) || outside.get(taskMap.get(tid)) != 1))  //RULE INS OUTSIDE
			  {
				  nodeId = ++countNode;
			  }
			  else 
			  ...
			  else if (line.contains(" CALL ")) 
			  {
				  ...
				  state.L.put(tid,v);	//check
					...
					if((!flagAtomicLife.containsKey(fk) || flagAtomicLife.get(fk)== 0) && (!outside.containsKey(taskid) || outside.get(taskid) == 0))
					{
						ls2.add(++newNode);
						state.H.put(nodeId, ls2);
				  ...
				  if (state.P.containsKey(taskid) && state.H.containsKey(state.P.get(taskid).node) && state.H.get(state.P.get(taskid).node) != null)
				  {
					 ...
					 // System.out.println(line+" node:"+nodeId);
					  //System.out.println(taskid+" taskid "+fk1.toString()+" fk1 and TaskM "+TaskM.toString());
					  if(/*flagAtomicLife.get(fk1)==0*/ outside.containsKey(taskid)  && outside.get(taskid) != 0)		//if call is outside atomic block add edge otherwise not
					  {
						  ls.add(state.P.get(taskid).node); 
						  //print(state,line,state.P.get(taskid).node,nodeId);
						  entered =1;
				...
				else if (line.contains("POST")) 
				{
					...
					//System.out.println("tid:"+tid+" L:"+state.L.get(tid).node+" "+state.L.get(tid).line);
				}
				else if (line.contains("RET")) 
				{
					taskMap.put(tid,-1);	//thread is idle
	...
	{	
		BlameAssign ba= new BlameAssign();
		String con = args[0];
		...
		for(i=1;i<=filenum;i++)
		{
			...
			concat = concat.concat("abc_log");
			concat = concat.concat(".txt");
			concat = concat.concat(temp);
			ba.phaseTree(concat);
			ba.phaseCheck(i,args[0]);
		}*/
		ba.phaseTree(con);
		//ba.phaseCheck();
		ba.markCalls(con);
		ba.checkLifecycles(con);
		
		System.out.println();
		System.out.println("priyanka is-------------------------------------------------------DONE-------------------------------------------------------------");
	}
}